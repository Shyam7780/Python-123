    ...
    def visit_Call(self, node):
     if isinstance(node.func, ast.Name):
        ...
        if func_name in self.functions:
            ...
            # Check true recursion
            if func_name == getattr(node, 'parent_func', None):
...
title = tk.Label(root, text="Advanced AI Code Complexity Analyzer", font=("Arial", 16), bg="Black", fg="White")
title.pack()

text_box = tk.Text(root, height=20, width=90, bg="Gray", fg="White")
text_box.pack()

text_box.bind("<KeyRelease>", auto_analyze)

analyze_btn = tk.Button(root, text="Analyze Code", command=analyze_code, bg="Gray", fg="White")
analyze_btn.pack(pady=10)

result_label = tk.Label(root, text="", justify="left", bg="Black", fg="White")
result_label.pack()
